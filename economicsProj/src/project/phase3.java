...
//one side is going to be stage2shareList.get(x) This will be the left side, right side is going to be stage2ShareListWithSwitch
  ...
  public void allocationPage(final List<Share> stage1, final List<Share> stage2, final double r, final int m){
    ...
    final JButton buttonFour = new JButton("View Prices | Stage 1");
    buttonFour.setBorder(new TitledBorder("Stage 1 Prices"));
    ...
    ActionListener actionChart = new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        if(source == buttonFour){
          ...
          JPanel mainPan = new JPanel(new GridLayout(stage1.size(), 1, 5, 5));

          for(int i=0; i<stage1.size(); i++){
            JLabel labels = new JLabel("Price of Security # " + (i+1) + " = " + Integer.toString(stage1.get(i).getPrice()));
            mainPan.add(labels);
          }
...
//        }
    ...
    buttonFour.addActionListener(actionChart);
    ...
    frame2.add(buttonFour, BorderLayout.NORTH);
